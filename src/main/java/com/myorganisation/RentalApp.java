...
import com.myorganisation.constants.PassengersLimit;
...
public class RentalApp {
	...
	private double getFinalRateIfPassengersExccededLimit(VehicleModel vehicleModel, double rate,
		...
		switch (vehicleModel) {
		case SUV:
			if (numberOfPassengers > PassengersLimit.SUV_LIMIT) {
				finalRate += numberOfPassengers;
			}
			break;
		case CAR:
			if (numberOfPassengers > PassengersLimit.CAR_LIMIT) {
				finalRate += numberOfPassengers;
			}
			break;
		case VAN:
			if (numberOfPassengers > PassengersLimit.VAN_LIMIT) {
				finalRate += numberOfPassengers;
			}
			break;
		case BUS:
			if (numberOfPassengers > PassengersLimit.BUS_LIMIT) {
				finalRate += numberOfPassengers;
			}
			break;
		}
